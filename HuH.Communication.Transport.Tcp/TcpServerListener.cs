...
using HuH.Communication.Common.Extensions;
...
namespace HuH.Communication.Transport.Tcp
{
    public class TcpServerListener
    {
        ...
        public TcpServerListener(IPEndPoint listeningEndPoint, IByteBufferPool receiveDataBufferPool, Action<ITcpConnection, byte[], Action<byte[]>> messageArrivedHandler)
        {
        ...
        private void OnSocketAccepted(Socket socket)
        {
            Task.Factory.StartNew(() =>
            {
                try
                {
                    var connection = new TcpConnection(socket, _receiveDataBufferPool, OnMessageArrived, OnConnectionClosed);
        ...
        {
            _connectionDict.Remove(connection.ConnectionId);
            foreach (var listener in _connectionEventListeners)
            {
                try
                {
